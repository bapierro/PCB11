#!/usr/bin/env python3
#This is done with copilot, only an initial version to test the data etc. 
"""
...
def _compute_rdm_from_features(features: np.ndarray, method: str = "correlation") -> np.ndarray:
    """Compute a 2D RDM from feature vectors.

    Currently supports:
    - "correlation": 1 - Pearson correlation between rows (items)
    """
    if features.ndim != 2:
    ...
    if method == "correlation":
        ...
        # features shape: (n_items, n_features)
        # correlation matrix: items x items
        corr = np.corrcoef(features)
        # convert to dissimilarity (1 - corr)
        dissim = 1.0 - corr
        return dissim
    else:
        raise ValueError(f"Unknown feature RDM method: {method}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--features-dir",
        type=Path,
        default=Path("features/baseline"),
        help="Optional directory with feature .npy files to compute ANN RDMs if no *_rdm.npy present.",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    if not rdm_files:
        # Try to compute RDMs from feature files
        features_dir = args.features_dir
        print(f"  No precomputed ANN RDMs found. Looking for feature files in {features_dir}...")
        feature_files = sorted(p for p in features_dir.glob("*.npy") if p.is_file())
        if not feature_files:
            raise SystemExit(f"No ANN RDMs found and no feature .npy files in {features_dir} to compute them.")

        ann_rdm_outdir = args.ann_rdms
        ann_rdm_outdir.mkdir(parents=True, exist_ok=True)
        rdm_files = []
        for feat_path in feature_files:
            if feat_path.name == "file_names.txt":
                continue
            print(f"  Computing RDM from features: {feat_path.name}...")
            feats = np.load(feat_path)
            try:
                rdm = _compute_rdm_from_features(feats, method="correlation")
            except Exception as e:
                print(f"    ⚠️  Skipping {feat_path.name}: {e}")
                continue
            out_path = ann_rdm_outdir / f"{feat_path.stem}_rdm.npy"
            np.save(out_path, rdm)
            rdm_files.append((feat_path.stem, out_path))
        print(f"  Computed and saved {len(rdm_files)} ANN RDMs to {ann_rdm_outdir}")
    else:
    ...
    if meg_rdm.ndim == 2:
        ...
        layers_file = output_dir / "layers.txt"
        layers_file.write_text("\n".join(layer_names) + "\n", encoding="utf-8")
        print(f"  ✅ Layer names: {layers_file.name}")

        # Save per-layer timecourses and CSVs
        perlayer_dir = output_dir / "per_layer"
        perlayer_dir.mkdir(parents=True, exist_ok=True)
        for i, name in enumerate(layer_names):
            arr = correlations_by_time[:, i]
            np.save(perlayer_dir / f"{name}_timecourse_{args.metric}.npy", arr)
            # CSV: time, correlation
            csv_path = perlayer_dir / f"{name}_timecourse_{args.metric}.csv"
            if time_vector is not None:
                lines = ["time,correlation\n"]
                for t, val in zip(time_vector, arr):
                    lines.append(f"{t},{val:.6f}\n")
            else:
                lines = ["time_index,correlation\n"]
                for idx, val in enumerate(arr):
                    lines.append(f"{idx},{val:.6f}\n")
            csv_path.write_text("".join(lines), encoding="utf-8")
        print(f"  ✅ Per-layer timecourses saved in: {perlayer_dir}")
